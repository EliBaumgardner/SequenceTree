                                          ...
                                          int lastTargetId, const NodeMap& nodes)
{
    ...
    else
    {
        ...
        if (parentIt != nodes.end())
        {
            ...
            if (durIt != parentIt->second.durationMap.end())
                                   ...
                                   NodeMap& nodes, TraversalMap& traversalMap)
{
    ...
    RTNode* nextTarget = traversalIt->second.peekNextTarget(nodes);
    int duration = resolveDuration(node, nextTarget, traversalIt->second.lastTargetId, nodes);
            ...
            || node.nodeType == RTNode::NodeType::Modulator))
    {
        ...
        bridge.pushProgress(node.nodeID, nextTarget->nodeID, wallClockMs, traversalIt->second.rootId);
    }
        ...
        || node.nodeType == RTNode::NodeType::RootNode)
    {
        for (int connectorId : traversalIt->second.peekTraversers(nodes))
        {
            ...
            if (connectorNode.nodeType == RTNode::NodeType::RootNode)
            {
                ...
                bridge.pushProgress(node.nodeID, connectorId, wallClockMs, traversalIt->second.rootId);
            }
            else
            {
                                            ...
                                            NodeMap& nodes, TraversalMap& traversalMap)
{
    ...
    if (type == RTNode::NodeType::RootNode && expiredNote.isConnectionTrigger)
    {
        ...
        if (traversal.shouldTraverse())
            pushRootNodeConnection(expiredNote.noteNode.nodeID, midiMessages,
    ...
    else if (type == RTNode::NodeType::Connector)
    {
        ...
        if (traversal.shouldTraverse())
            pushConnectorNotes(expiredNote.noteNode.nodeID, midiMessages,
                               priorityNoteDuration, nodes, traversalMap);
    }
    else if (type == RTNode::NodeType::ModulatorRoot)
    {
        ...
        if (traversal.shouldTraverse())
            pushModulatorNotes(expiredNote.noteNode.nodeID, midiMessages,
                               priorityNoteDuration, nodes, traversalMap);
    }
}
